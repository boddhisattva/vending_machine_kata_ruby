...
RSpec.describe ItemReloader do
  ...
  describe '#reload_items_for_machine' do
    before do
    ...
    context 'when reloading existing item' do
      it 'adds quantity to existing item' do
        ...
        allow(vending_machine).to receive(:reload_item).with('Coke', 3)
        ...
        expect { reloader.reload_items_for_machine }.to output(expected_output).to_stdout
      end

      it 'handles invalid quantity gracefully' do
        allow(input_handler).to receive(:safe_gets).and_return('Coke', '0')
        allow(vending_machine).to receive(:reload_item).with('Coke', 0)
    ...
    context 'when adding new item' do
      it 'asks for price and adds new item' do
        ...
        allow(vending_machine).to receive(:reload_item).with('Pepsi', 5, 175)
      ...
      it 'handles invalid price gracefully' do
        ...
        allow(vending_machine).to receive(:reload_item).with('Pepsi', 5, -100)
