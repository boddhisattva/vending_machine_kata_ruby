  ...
  describe '#reload_item' do
    context 'reloading existing item' do
      it 'increases quantity and returns success message' do
        result, updated_items = manager.reload_item(items, items_index, 'Coke', 5)
      ...
      it 'ignores price parameter for existing items' do
        result, = manager.reload_item(items, items_index, 'Coke', 3, 200)
    ...
    context 'adding new item' do
      it 'adds new item with correct attributes' do
        result, updated_items = manager.reload_item(items, items_index, 'Water', 10, 125)
    ...
    context 'validation errors' do
      it 'returns error for invalid quantity' do
        result, returned_items = manager.reload_item(items, items_index, 'Coke', -5)
      ...
      it 'returns error for missing price on new item' do
        result, returned_items = manager.reload_item(items, items_index, 'Water', 5)
    ...
    context 'with custom validator' do
      ...
      it 'uses injected validator' do
        ...
        manager_with_validator.reload_item(items, items_index, 'Coke', 5)
      end
    end
  end
