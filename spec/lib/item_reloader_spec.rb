  ...
  describe '#reload_item' do
    context 'reloading existing item' do
      it 'increases quantity and returns success message' do
        result, updated_items = reloader.reload_item(items, items_index, 'Coke', 5)
      ...
      it 'ignores price parameter for existing items' do
        result, = reloader.reload_item(items, items_index, 'Coke', 3, 200)
    ...
    context 'adding new item' do
      it 'adds new item with correct attributes' do
        result, updated_items = reloader.reload_item(items, items_index, 'Water', 10, 125)
    ...
    context 'validation errors' do
      it 'returns error for invalid quantity' do
        result, returned_items = reloader.reload_item(items, items_index, 'Coke', -5)
      ...
      it 'returns error for missing price on new item' do
        result, returned_items = reloader.reload_item(items, items_index, 'Water', 5)
    ...
    context 'with custom validator' do
      ...
      let(:reloader_with_validator) { ItemReloader.new(mock_validator) }
      ...
      it 'uses injected validator' do
        ...
        reloader_with_validator.reload_item(items, items_index, 'Coke', 5)
      end
    end
  end
end
